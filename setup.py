...
class Game:
    def __init__(self):
        ...
        pygame.display.set_caption('Moonbeam Adventure')
        ...
        self.sfx = {
            'jump': pygame.mixer.Sound('Aset/sound_effect/jump.mp3'),
            'dash': pygame.mixer.Sound('Aset/sound_effect/dash.mp3'),
            #'hit': pygame.mixer.Sound('Aset/sound_effect/sfx/hit.wav'),
            #'shoot': pygame.mixer.Sound('Aset/sound_effect/sfx/shoot.wav'),
            'ambience': pygame.mixer.Sound('Aset/sound_effect/Backsound/inGame.mp3'),
        }
    ...
    def load_level(self, map_id):
        self.tilemap.load('Aset/Maps/' + str(map_id) + '.json')
    ...
    def run(self):
        ...
        pygame.mixer.music.load('Aset/sound_effect/Backsound/inGame.mp3')
        ...
        while True:
            ...
            background = pygame.transform.smoothscale(self.assets['background'], self.display.get_size())
            self.display.blit(background, (0, 0))
            
            if not len(self.enemies):
                self.transition += 1
                if self.transition > 30:
                    self.level = min(self.level + 1, len(os.listdir('Aset/maps')) - 1)
                    self.load_level(self.level)
            ...
            if self.player.check_collision_with_flag():
                self.transition += 1
                if self.transition > 30:
                    self.level = min(self.level + 1, len(os.listdir('Aset/maps')) - 1)
                    self.load_level(self.level)
